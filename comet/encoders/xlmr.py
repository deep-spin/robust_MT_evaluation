...
from transformers import XLMRobertaModel, XLMRobertaTokenizer, BertTokenizer
...
class XLMREncoder(BERTEncoder):
    ...
    def extend_wl_tags(self, pretrained_model: str, model_type: str):
        if model_type == 'wl-tags':
            special_tokens_dict = {'additional_special_tokens': ['<BAD>', '<OK>']}
            num_added_tokens = self.tokenizer.add_special_tokens(special_tokens_dict)
            # print('! ALL special toks !: ', self.tokenizer.all_special_tokens)
            # print('num: ', num_added_tokens)
            self.model = XLMRobertaModel.from_pretrained(
                pretrained_model, add_pooling_layer=False
            )
