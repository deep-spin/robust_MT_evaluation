...
from comet.modules import HTSLoss, HTSApproxLoss, HTSLoss2, HTSApproxLossv2
...
def score_command() -> None:
    ...
    if cfg.gpus > 1 and cfg.accelerator == "ddp":
        ...
        seg_scores, std_scores, hts_scores, hts_std_scores, sys_scores = [], [], [], [], []
        new_data = []
        #print(data.keys())
        #print('=-=-=-=-=-=-=-=-=-=-=-=-=-=-=')
        for i in range(len(cfg.translations)):
            #print(len(cfg.translations))
            ...
            print(outputs.keys())
            if cfg.mc_dropout > 0 and ( isinstance(model.loss, HTSLoss) or isinstance(model.loss, HTSLoss2)):
                seg_scores.append(outputs.score)
                std_scores.append(outputs.std)
                hts_scores.append(outputs.hts)
                hts_std_scores.append(outputs.hts_std)
            elif cfg.mc_dropout > 0 or isinstance(model.loss, HTSLoss) or isinstance(model.loss, HTSLoss2):
                seg_scores.append(outputs.score)
                std_scores.append(outputs.std)
            else:
                seg_scores.append(outputs.score)
    ...
    for i in range(len(data[files[0]])):  # loop over (src, ref)
        for j in range(len(files)):  # loop of system
            ...
            if cfg.mc_dropout > 0 and ( isinstance(model.loss, HTSLoss) or isinstance(model.loss, HTSLoss2)):
                ...
                data[files[j]][i]["hts"] = hts_scores[j][i]
                data[files[j]][i]["hts_variance"] = hts_std_scores[j][i]
                if not cfg.quiet:
                    print(
            ...
            elif cfg.mc_dropout or isinstance(model.loss, HTSLoss) or isinstance(model.loss, HTSLoss2):
                data[files[j]][i]["variance"] = std_scores[j][i]
                if not cfg.quiet:
                    print(
                        "{}\tSegment {}\tscore: {:.4f}\tvariance: {:.4f}".format(
