...
def score_command() -> None:
    ...
    parser.add_argument("-wlt", "--word_level_tags", type=Path_fr, help="Path to word-level tags for predictor (optional)")
    ...
    #     default="original",
    #     help=(
    #         "Type of model. There are two options: 'original' and 'wl-tags'."
    #         + " 'original' is a baseline comet model, can be used for training with sl-features or aug data."
    #         + " 'wl-tags' is used if the training set includes wl tags."
    #     )
    # )
    ...
    if cfg.word_level_tags is not None :
        print('Inference time. Reading and converting MT tags into speial tokens...')
        with open(cfg.word_level_tags(), encoding="utf-8") as fp:
            lines = [line.strip() for line in fp.readlines()]
            word_level_tags = convert_word_tags(lines)
    ...
    if model.is_referenceless():
        ...
        if cfg.features is not None :
            ...
            for i,f in enumerate(features):
                data['f'+str(i+1)] = [f for _ in translations]

        elif cfg.word_level_tags is not None :
            data = {
                "src": [sources for _ in translations],
                "mt": translations,
                "ref": [references for _ in translations],
                "mt_tags": [word_level_tags for _ in translations],
            }
        else:
            # data = {"src": sources, "mt": translations, "ref": references}
            data = {
